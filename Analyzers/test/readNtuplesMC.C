...
//   std::cout << "input file: " << inputfile -> GetName() << std::endl;
  ...
  for (int i = 0; i < 100; i++){
    tree -> Add(Form("root://eoscms//eos/cms/store/group/phys_muon/fiorendi/13TeV/MC/efficiency_iterL3PlusL1_v12/DYJetsToLL_M-50_TuneCUETP8M1_13TeV-madgraphMLM-pythia8/efficiency_iterL3PlusL1_v12_DY/170518_143437/0000/muonNtuple_%d.root/muonNtuples/muonTree", i));
  }

  TFile* outfile = TFile::Open("efficiency_onDYToLL_TSG83X_v12.root","RECREATE");
...
//         if (!(offlineiso04 < 0.15)) continue;
        if (! matchMuon(probeMu, ev -> hlt.objects, "hltL1fL1sMu22Or25L1Filtered0::REHLT")) continue;
...
//         if (matchMuon(probeMu, ev -> hlt.objects, "hltL3crIsoL1sMu16L1f0L2f10QL3f20QL3trkIsoFiltered0p09::REHLT")) pass = true;
        ...
        HLTMuonCand theL3 = matchL3(probeMu, ev -> iterL3muons);
...
//         foundL3 = true;
        ...
        if (matchMuon(probeMu, ev -> hlt.objects, "hltL3fL1sMu25f0Filtered25Q::REHLT")) foundL3 = true;
...
//         if (!foundL3) continue;  
        ...
        muonPt      -> Fill( foundL3    , probeMu.pt  );

        if (probeMu.pt > cut_pt_offline ){
          muonEta       -> Fill( foundL3,      probeMu.eta );
          muonPhi       -> Fill( foundL3,      probeMu.phi );
          nvtx          -> Fill( foundL3,      ev -> nVtx  );

        }
        if (isBarrel){
          muonPt_barrel    -> Fill( foundL3, probeMu.pt  );
          if (probeMu.pt > cut_pt_offline ) {
            nvtx_barrel        -> Fill( foundL3  , ev -> nVtx   );
          }
        }
        
        else{
          muonPt_endcap    -> Fill( foundL3, probeMu.pt  );
          if (probeMu.pt > cut_pt_offline ){
            nvtx_endcap       -> Fill( foundL3, ev -> nVtx);
          }
        }
